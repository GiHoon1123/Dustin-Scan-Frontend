...
export default function TransactionDetailPage() {
  ...
  const view = searchParams.get("view") === "logs" ? "logs" : "overview";

  return (
    <div className="container mx-auto px-4 py-4 md:py-8">
      <DataLoader
        cacheKey={CacheKeys.transactionByHash(hash)}
        loadData={async () => {
          const txData = await getTransactionByHash(hash);
          return txData.data as Transaction;
        }}
        render={(tx, isLoading, fromCache) => {
          if (isLoading) {
            return (
          ...
          return (
            <>
              <div className="flex items-center justify-between mb-4 md:mb-6">
                ...
                {fromCache && (
                  <CacheIndicator cacheKey={CacheKeys.transactionByHash(hash)} />
                )}
              </div>
